...
# data subdirectory 
INROOT = '/mnt/hgfs/Data/thesis_data/tob_pv_estimation/HCP_retest/test'
OUTROOT = '/mnt/hgfs/Data/thesis_data/tob_projection/projected_output'
N_SUBS = 45
REF = op.join(OUTROOT, 'ref_2.2.nii.gz') # reference space for all tests 
REF_SPC = tob.ImageSpace(REF) # same again, as object 
...
def make_surf_truths(spc, surf):
    i,j,k = [ np.arange(0, spc.size[dim]) for dim in range(3) ]
    interp_points = aff_trans(spc.world2vox, surf.points)
...
def test_subject(data_dict, surf_dict, subid, exp, surf_paths):
    ...
    tp_name = op.join(OUTROOT, 'projectors', '%s_tob.h5' % subid)
    projector = tob.Projector.load(tp_name)

    # Naive: volume-surface projection only, no edge effects
    # In the vol->surf direction, edge scaling INCREASES signal
    # In the surf->vol direction, edge scaling DECREASES signal
    if exp == 'naive':
        v2n_tob_mat = projector.vol2surf_matrix(edge_scale=False)
        n2v_tob_mat = projector.surf2vol_matrix(edge_scale=True)

    # Volume-surface, with edge effects
    elif exp == 'edge_corr':
        v2n_tob_mat = projector.vol2surf_matrix(edge_scale=True)
        n2v_tob_mat = projector.surf2vol_matrix(edge_scale=True)

    # Volume-node, with edge effects
    else:  
        v2n_tob_mat = projector.vol2node_matrix(edge_scale=True)
        n2v_tob_mat = projector.node2vol_matrix(edge_scale=True)
    ...
    v2n_wb_mat = tob.utils.sparse_normalise(wb_mat, 1)
...
if __name__ == "__main__":
    ...
    for exp in EXPERIMENTS:
        for idx, subid in enumerate(SUBIDS()[:N_SUBS]):
            ...
            p_name = op.join(OUTROOT, 'projectors', '%s_tob.h5' % subid)
            if not op.exists(p_name):
                L_hemi = tob.Hemisphere(L_surfs[0], L_surfs[2], 'L')
                proj = tob.projection.Projector(L_hemi, REF_SPC, FACTOR, cores=14)
                proj.save(p_name)
            else: 
                proj = tob.Projector.load(p_name)
            ...
            if not op.exists(smask_path):
                insurf = tob.Surface(L_surfs[0])
                outsurf = tob.Surface(L_surfs[2])
